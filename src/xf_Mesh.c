...
xf_CreateMeshParallelInfo( xf_MeshParallelInfo **pParallelInfo){
  ...
  (*pParallelInfo)->nIFaceHalo    = -1;
  (*pParallelInfo)->IFaceLoc2Glob = NULL;
  (*pParallelInfo)->nSendIFace    = NULL;
  (*pParallelInfo)->SendIFace     = NULL;
  (*pParallelInfo)->nRecvIFace    = NULL;
...
xf_DestroyMeshParallelInfo( xf_Mesh *Mesh, xf_MeshParallelInfo *ParallelInfo){
  ...
  xf_Release ( (void  *) ParallelInfo->nSendIFace);
  xf_Release2( (void **) ParallelInfo->SendIFace);
  xf_Release ( (void  *) ParallelInfo->nRecvIFace);
		   ...
		   int egR, int elemR, int faceR)
{
  ...
  xf_GetFaceGroup(Mesh, xfe_FaceInterior, -1, xfe_False, &Group);
  Mesh->ElemGroup[egL].Face[elemL][faceL].Group  = Group;
  Mesh->ElemGroup[egL].Face[elemL][faceL].Number = Mesh->nIFace;
  Mesh->ElemGroup[egR].Face[elemR][faceR].Group  = Group;
		  ...
		  enum xfe_ShapeType Shape, int nnode, int *fvec)
{
  /* This is not an optimized function.  Primary mode of operation for
     ...
     only.*/
  int ierr, i, egrp0, elem0, face0, nface0;
  int nfnode, pos1, pos2, nface, *nFaceNew;
  int iface1, iface2, Group;
...
int 
xf_ReadGriFile( const char *InputFile, const char **InputStrings, xf_Mesh *Mesh)
{
  int ierr, nNode, nElemTot, dim, nLocFaceMax;
  int i, k, nBFG, iBFG, nBFace, nfnode, Group;
  ...
  for (iBFG=0; iBFG<nBFG; iBFG++){
    ...
    if (ierr != 3){
      sprintf(title, "BoundaryGroup%d", iBFG);
	...
	if ((fvec[i] < 0) || (fvec[i] >= nNode)){
      ...
      // get face group info
      xf_GetFaceGroup(Mesh, xfe_FaceBoundary, iBFG, xfe_False, &Group);

      ierr = xf_Error(xf_AddFaceToHash(Node2Hash, nfnode, fvec, xfe_True, Group, 
  ...
  while (nElemCurrent < nElemTot){
	...
	ierr = xf_LinearElemJacobian(QBasis, QOrder, nvec, Mesh->Coord[0], NULL, &Volume);
	...
	if (exists){ // face already exists in hash list
	  ...
	  if (hinfo->BFlag == xfe_True){ // boundary face
            xf_FaceGroupInfo(Mesh, hinfo->Group, NULL, &ibfgrp, NULL);
	    xf_InitBFace(Mesh->BFaceGroup[ibfgrp].BFace + hinfo->Face);
	    Mesh->BFaceGroup[ibfgrp].BFace[hinfo->Face].ElemGroup = egrp;
	    Mesh->BFaceGroup[ibfgrp].BFace[hinfo->Face].Elem      = elem;
	    Mesh->BFaceGroup[ibfgrp].BFace[hinfo->Face].Face      = face;
...
xf_WriteGriFile( xf_Mesh *Mesh, char *OutputFile )
{
  ...
  xf_Call(xf_GetnElem(Mesh, NULL, &nelemtot));
  ...
  for (ibfgrp=0; ibfgrp<Mesh->nBFaceGroup; ibfgrp++){
    if (Mesh->BFaceGroup[ibfgrp].nBFace > 0){
      ...
      // local nodes on face
      xf_Call(xf_Q1NodesOnFace(Mesh->ElemGroup[egrp].QBasis, Mesh->ElemGroup[egrp].QOrder, 
                               face, &nfnode, fvec));
    }
    else nfnode = 0;
...
xf_ReadMeshBinary( FILE *fid, xf_Mesh *Mesh)
{
  ...
  ierr = xf_Error(xf_ParallelizeMesh(Mesh_Glob, NULL, NULL, Mesh));
  if (ierr != xf_OK) return ierr;
