...
/* Face groups fall into one of the following categories */
enum xfe_FaceType { 
  xfe_FaceInterior,   // interior face group 
  xfe_FaceBoundary,   // boundary face group
  xfe_FaceNull,       // null group, does not actually exist
  xfe_FaceLast
};
static char *xfe_FaceName[xfe_FaceLast] = {
  "Interior",
  "Boundary",
  "Null"
};
...
/* Mesh Parallel Info structure; not read or written */
typedef struct
{

  /*** IFace parallelization information ***/
  ...
  /* Number of regular interior faces in the mesh, where a regular
     ...
     processor.  The alternative would be faces for which one of the
     adjacent elements is in a halo group.  Regular IFaces are listed
     first in the local mesh IFace list, so nIFaceRegular is used to
     know when to pause to check for and allow inter-processor element
     data communication to complete, when for example computing the
     iface residuals in DG. */

  /* Note, Mesh->nIFace stores the number of "self" faces, which is
     the number of interior faces that belong to this processor.
     This number is at least as large as nIFaceRegular.  Some of these
     "self" interior faces have halo elements as neighbors: the
     decision of on which processor the straddling face sits is made
     during parallelization of the interior faces.  This is used for
     interior-face-based data communication. */

  int nIFaceHalo;
  /* Number of halo interior faces.  These are faces on a processor
     that do not belong to the processor but are adjacent to a self
     face, meaning that there exists an element that links these halo
     faces to a self face. */
  ...
  int *nSendIFace;
  /* nSendIFace[iProc] stores the number of ifaces that serve as halo
     ifaces on processor iProc.  It is used jointly with SendIFace. */

  int **SendIFace;
  /* SendIFace[iProc][i], where 0 <= i < nSendIFace[iProc] stores the
     iface numbers of those interior faces that serve as halo cells
     for processor iProc.  When it comes time to exchange iface data,
     the current processor will send data for these nSendIFace[iProc]
     ifaces to the appropriate halo location on processor iProc. */

  int *nRecvIFace;
  /* nRecvIFace[iProc] stores the number of halo ifaces on current
     processor that reside on processor iProc. The current processor
     expects to receive data on these faces during a data
     communication. */
  ...
  /*** Element parallelization information ***/
