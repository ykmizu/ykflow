...
void yk_fomBuffer(yk_PrimalSolver *ykflow, Universe equation, Is_it *reduced, \
  ...
  sprintf(nameOfDir, "%s_M_%d_A_%d_Re_%d", jobFile, mach, alfa, reynolds);
}
...
void yk_xflow_totalResidual(yk_PrimalSolver *ykflow, Multiverse *multiquation,
  ...
  xf_Call(xf_GetTimeStep(xflow->TimeHistData, iTime, &TimeScheme, &Time, &TimeStep));
  printf("%d %g %d %d\n", iTime, Time, TimeScheme, TimeStep);
  printf("seeeeeeep\n");
  //---------------------------------------------------------------------------
  // Implementation
  //---------------------------------------------------------------------------
  /* xf_Call(xf_GetKeyValue(xflow->KeyValueArg, "job", jobFile)); */
  /* sprintf(SavePrefix, "%s", "naca_window_0"); */
  /* printf("%s\n", SavePrefix); */
...
void yk_EqnSetYkflowInitialize(yk_PrimalSolver *ykflow, Multiverse *multiquation,
...
void yk_RunSimulation(yk_PrimalSolver *ykflow, Multiverse *multiquation,
		      Cluster *fom, Cluster *rom, Cluster *hrom, Is_it *reduced){
  int i, j;
  int ierr;
  char SavePrefix[xf_MAXSTRLEN];
  ...
  char LogOutput[xf_MAXSTRLEN];
  char LogOutput_i[xf_MAXSTRLEN];
  real StartTime, WindowSize, EndTime;
  real WindowStart, WindowEnd;
  xf_Data *D;

  double dt = fom->self->time.dt;
  int nTimeStep;
  xf_VectorGroup *UG0;
  int iTime;
  real Time;
  ...
  enum xfe_TimeSchemeType TimeScheme;
  xf_Call(xf_GetKeyValue(xflow->All->Param->KeyValue, "SavePrefix", SavePrefix));
  xf_Call(xf_GetKeyValue(xflow->All->Param->KeyValue, "SavePrefix", name));

  // write initial time window node primal state to disk
  sprintf(FileName, "%s_Window00_U.data", SavePrefix);
  xf_Call(xf_DumpVectorGroupBinary(xflow->All->Mesh, "State", xflow->UG, FileName));

  StartTime = fom->self->time.t_0;
  EndTime = fom->self->time.t_f;
  printf("%d\n", reduced->nTimeSegs);
  WindowSize = (EndTime-StartTime)/reduced->nTimeSegs;
  printf("%g\n", WindowSize);
  nTimeStep = (int)(WindowSize/fom->self->time.dt);
  xf_Call(xf_GetKeyValue(xflow->All->Param->KeyValue, "LogOutput", LogOutput));
  for (i=0; i<reduced->nTimeSegs; i++){
    sprintf(SavePrefix, "%s_window_%d", name, i);
    strcpy(multiquation->equation.nameEqn, SavePrefix);
    strcpy(fom->self->id, SavePrefix);
    /* sprintf(LogOutput_i, "%s_%i", LogOutput, i); */
    xf_Call(xf_SetKeyValue(xflow->All->Param->KeyValue, "SavePrefix",SavePrefix));
    /* xf_Call(xf_GetKeyValue(xflow->All->Param->KeyValue, "LogOutput", LogOutput)); */

    xf_Call(xf_FindPrimalState(xflow->All->DataSet, 0, &D, NULL));
    xflow->UG = (xf_VectorGroup *) D->Data;

    WindowStart = StartTime + i*WindowSize;
    WindowEnd = StartTime + (i+1)*WindowSize;
    fom->self->time.t_0 = WindowStart;

    fom->self->time.t_f = WindowEnd;
    fom->self->time.count = (WindowEnd - WindowStart)/dt;
    /* fom->self->time.count = (int) WindowSize/dt; */
    printf("%g %g\n", WindowStart, WindowEnd);

    // create a uniform time history
    xf_Call(xf_SetKeyValueInt(xflow->All->Param->KeyValue, "nTimeStep", nTimeStep));
    xf_Call(xf_SetKeyValueReal(xflow->All->Param->KeyValue, "Time", WindowStart));
    xf_Call(xf_SetKeyValueReal(xflow->All->Param->KeyValue, "EndTime", WindowEnd));
    xf_Call(xf_CreateUniformTimeHistData(xflow->All, LogOutput, &xflow->TimeHistData));
    xf_Call(xf_SetKeyValueInt(xflow->All->Param->KeyValue, "UnsteadyWriteInterval", 1));

    // call primal solver
    xf_printf("\n** Running Window %d: StartTime = %.5e, EndTime = %.5e, nTimeStep = %d **\n",
              i, WindowStart, WindowEnd, nTimeStep);
    xf_printf("\nCalling primal solver.\n");
    xf_Call(xf_ApplyTimeScheme(xflow->All, SavePrefix, xfe_False, &xflow->UG, xflow->TimeHistData)
);

    /* xf_Call(xf_SetKeyValueReal(xflow->All->Param->KeyValue, "Time", 0.1)); */
    /* printf("%d %g %g\n", iTime, Time, TimeStep); */
    /* getchar(); */

    for (j=0; j<fom->self->time.count+1; j++){
      sprintf(OutputFile, "%s_U%d.xfa", SavePrefix, j);
      xf_Call(xf_WriteAllBinary(xflow->All, OutputFile));
    }
    ...
    sprintf(FileName, "%s_WindowNode%02d_U.data", SavePrefix, i+1);
    xf_Call(xf_DumpVectorGroupBinary(xflow->All->Mesh, "State", xflow->UG, FileName));

    // store (end of window) UG away for use after adjoint (not stored in All)
    xf_Call(xf_FindSimilarVectorGroup(xflow->All, xflow->UG, "NotInAll", xfe_True, xfe_False, NULL, &UG0, NULL));
    xf_Call(xf_SetVectorGroup(xflow->UG, xfe_Set, UG0));
    UG0->Vector[0]->TimeIndex = 0; // make look like primal solver state
    UG0->Vector[0]->SolverRole = xfe_SolverRolePrimalState;

    xf_Call(xf_DestroyTimeHistData(xflow->TimeHistData));

    // put end-of-time window primal state into All for use in next window
    xf_Call(xf_DataSetRemove(xflow->All->DataSet, "State", xfe_True));
    xf_Call(xf_DataSetAdd(xflow->All->DataSet, "State", xfe_VectorGroup, xfe_True, (void *) UG0, NULL));
...
/*   int ierr; */
/*   enum xfe_Bool DoRestart; */
/*   char jobFile[xf_MAXSTRLEN], OutputFile[xf_MAXSTRLEN]; */
...
/*   yk_EqnSetYkflowInitialize(ykflow, multiquation, fom, reduced); */
/*   /\* yk_forwardSimulation(xflow); *\/ */



/*   yk_RunSimulation(ykflow, multiquation, fom, reduced); */
...
void yk_RunErrorEstChaos(yk_PrimalSolver *ykflow, Multiverse *multiquation,
  ...
  yk_RunSimulation(ykflow, multiquation, fom, rom, hrom, reduced);
...
/* //------------------------------------//------------------------------------- */
  ...
  /* xf_Data *D; */
  /* xf_VectorGroup *UG0, **PsiG=NULL; */
  /* real WindowStart, WindowEnd, WindowTime; */
  /* xf_VectorGroup *UG; */
  /* /\* printf("Number of time windows to calculate at: %d\n", reduced->nTimeSegs); *\/ */
  /* WindowTime=(fom->self->time.t_f-fom->self->time.t_0)/reduced->nTimeSegs; */
  /* int wonderboy = WindowTime/fom->self->time.dt; */

  /* printf("%d\n", wonderboy); */
  /* getchar(); */

  /* xf_Call(xf_GetKeyValueBool(xflow->All->Param->KeyValue, "Restart", &DoRestart)); */
  /* xf_Call(xf_FindOrCreatePrimalState(xflow->All, 1, NULL, &xflow->UG)); */
...
/*     WindowStart = fom->self->time.t_0; */
/*     WindowEnd = fom->self->time.t_0 + (1)*WindowTime; */
/*     fom->self->time.window_i = i; */


/*     xf_Call(xf_SetKeyValueInt(xflow->All->Param->KeyValue, "nTimeStep", wonderboy)); */
/*     xf_Call(xf_SetKeyValueReal(xflow->All->Param->KeyValue, "Time", WindowStart)); */
/*     xf_Call(xf_SetKeyValueReal(xflow->All->Param->KeyValue, "EndTime", WindowEnd)); */
/*     xf_Call(xf_CreateUniformTimeHistData(xflow->All,LogOutput, &xflow->TimeHistData)); */
/*     xf_Call(xf_SetKeyValueInt(xflow->All->Param->KeyValue, "UnsteadyWriteInterval", 1)); */
...
/*     // call primal solver */
/*     xf_printf("\n** Running Window %d: StartTime = %.5e, EndTime = %.5e, TimeStep = %.5e, nTime\ */
/* Step = %d **\n", */
/*               i, WindowStart, WindowEnd, fom->self->time.dt, wonderboy); */
/*     xf_printf("\nCalling primal solver.\n"); */
/*     /\* xf_Call(xf_ApplyTimeScheme(xflow->All, SavePrefix, xfe_False, &xflow->UG, xflow->TimeHistData)); *\/ */
...
/*     printf("        --------------------------------------------------------------\n"); */
/*     /\* xf_Call(xf_GetTimeStep(xflow->TimeHistData, iTime, &TimeScheme, &Time, &TimeStep)); *\/ */
/*     /\* printf("%d %g %g %d\n", iTime, TimeScheme, Time, TimeStep); *\/ */
/*     yk_createReducedOrderModel_ST(ykflow, multiquation, fom, reduced); */
/*     yk_runReducedOrderModel_ST(ykflow, multiquation, fom, rom, reduced); */
